...
def init_db():
    ...
    """)
    # Purge records older than 24 hours so old tenders don't block re-appearance
    conn.execute("""
        DELETE FROM seen
        WHERE added < datetime('now', '-24 hours')
    """)
    conn.commit()
    return conn
...
def extract_fields(raw_text, source_url=""):
    ...
    deadline = "N/A"
    deadline_patterns = [
        r"(?:Due\s*Date|Last\s*Date|Submission\s*(?:Date|Deadline)|Closing\s*Date|End\s*Date|Bid\s*(?:End|Close)|Deadline)\s*[:\-]?\s*(\w+\s+\d{1,2}\s*,?\s*\d{4})",
...
# ─────────────────────────────────────────────────────────────────────
# DATE FILTER — Only keep tenders relevant to today
# ─────────────────────────────────────────────────────────────────────
MONTH_MAP = {
...
def parse_date(date_str):
    ...
    # Normalize "May 25 , 2026" → "May 25, 2026"
...
def is_recent(tender, days=7):
    """
    Keep tender if:
    1. Deadline is today or in the future (not expired)
    2. OR deadline is unknown (N/A) — don't discard, might be new
    Reject if deadline is clearly in the past (more than 1 day ago).
    """
    deadline_str = tender.get("deadline", "N/A")

    if deadline_str == "N/A":
        # No date found — keep it, could be genuinely new
        return True
    ...
    # Reject if deadline already passed by more than 1 day
    cutoff = datetime.now() - timedelta(days=1)
    if dt < cutoff:
        log.debug(f"FILTERED (expired {deadline_str}): {tender.get('title','')[:60]}")
        return False
...
def main():
    ...
    # Filter out expired/old tenders (deadline already passed)
    fresh = [t for t in matched if is_recent(t)]
    log.info(f"Fresh (deadline not expired): {len(fresh)} of {len(matched)}")

    # Deduplicate — only alert on tenders not seen in last 24 hours
