...
def make_tender(title, link, source, base_url=""):
    if not link.startswith("http") and base_url:
        link = base_url + link
    return {"title": title[:350], "link": link, "source": source}
...
def parse_table_rows(soup, source, base_url, col_title=1, col_link=1):
    ...
    for row in soup.find_all("tr"):
        ...
        if len(cols) > col_title:
            ...
            full_title = f"{t} [{dept}]" if dept and dept != t else t
            if len(t) > 10:
                results.append(make_tender(full_title, link, source, base_url))
    return results
...
def scrape_gem():
    ...
    if not r:
        log.warning("GeM BidPlus: blocked/unreachable")
        return results
    ...
    for card in soup.find_all("div", class_=lambda c: c and "bid" in str(c).lower())[:50]:
        ...
        if len(t) > 15:
            results.append(make_tender(t, link, "GeM BidPlus ★"))
...
def send_email(tenders):
    ...
    def section(items, heading, bg):
        ...
        html = f"""
        <tr>
          <td colspan="2" style="background:{bg};color:#fff;padding:10px 16px;
        ...
        for src, sitems in by_src.items():
            ...
            html += f"""
        <tr>
          <td colspan="2" style="background:{color};color:#fff;padding:6px 16px;
              font-size:12px;font-weight:bold;">
            {src} — {len(sitems)} tender(s)
          </td>
        </tr>"""
            for t in sitems:
                kws = " · ".join(t.get("hits", [])[:5])
                html += f"""
        <tr style="border-bottom:1px solid #eee;vertical-align:top;">
          ...
          <td style="padding:10px 8px;font-size:11px;color:#777;">{kws}</td>
        </tr>"""
        return html

    rows  = section(govt, "🏛️ DIRECT GOVT PORTALS", "#1a237e")
    rows += section(agg,  "📋 AGGREGATOR SITES",    "#37474f")

    html = f"""<!DOCTYPE html><html><body
    style="font-family:Arial,sans-serif;background:#ececec;margin:0;padding:16px;">
    <div style="max-width:740px;margin:auto;background:#fff;border-radius:10px;
      ...
      <!-- Source count bar -->
      <div style="padding:10px 20px;background:#e8eaf6;font-size:11px;
           color:#3949ab;line-height:1.8;">
        <strong>20 Sources Monitored:</strong><br>
        🏛️ GeM BidPlus · CPPP/eProcure · eTenders NIC · ISTM · NCERT/NIOS ·
        NSDC/Skill India · NIELIT · BIS · DOPT/Karmayogi · MoD/DRDO ·
        AP eProcure · MahaTenders · Web Search (Govt)<br>
        📋 TenderDetail · TendersOnTime · BidAssist · NationalTenders ·
        FirstTender · TenderTiger · TenderDekho
      </div>
        ...
        <thead>
          <tr style="background:#f5f5f5;">
            <th style="padding:8px 16px;text-align:left;font-size:11px;
                color:#888;border-bottom:2px solid #ddd;">TENDER</th>
            <th style="padding:8px 8px;text-align:left;font-size:11px;
                color:#888;border-bottom:2px solid #ddd;">KEYWORDS</th>
          </tr>
        </thead>
        <tbody>{rows}</tbody>
      </table>
      ...
      <!-- Footer -->
      <div style="padding:14px 20px;background:#f5f5f5;font-size:11px;
           color:#aaa;border-top:1px solid #eee;">
        Novac Technology Solutions — Tender Monitor v6 | GitHub Actions (Free · 4x Daily)
      </div>
    </div></body></html>"""
